...
const Company = ({ data }) => {
  const [companies, setCompanies] = useState([...data.data]);
  ...
  return (
    <Layout>
      <div className="max-w-2xl mx-auto mb-5">
        <h1 className="text-3xl font-bold text-center mt-8 mb-4">Companies</h1>
        {/* Filter */}
          ...
          {companies.map((company) => {
            return (
              <div className="basis-1/4 bg-white">
                ...
                <Link href={`/seeker/company/${company.id}`}>
...
export const getServerSideProps = async (context) => {
  ...
  const result = await fetch('http://localhost:8000/companies', {
