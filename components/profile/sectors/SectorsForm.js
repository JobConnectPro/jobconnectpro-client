  ...
  return (
    <div className="w-full p-8 mt-10 ">
      <div className="w-1/2 mx-auto p-4">
        <h1 className="text-2xl text-center text-blue-700">Admin CRUD sectors</h1>
        <div className="flex justify-between items-center py-2 bg-slate-300 text-xl">
          <p>Sectors</p>
          <p>Action</p>
        </div>
        {sectors.map((sector) => (
          <table key={sector.id} class="w-full table-fixed">
            <tbody>
              <tr className="flex justify-between items-center bg-yellow-50 border-t border-slate-300">
                <td className="">{sector.sector}</td>
                <td>
                  <button className="bg-red-300 p-2" onClick={() => handleDelete(sector.id)}>
                    Delete
                  </button>
                  <button
