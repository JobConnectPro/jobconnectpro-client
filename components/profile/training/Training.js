...
const Training = ({ profile, isAdd, setIsAdd, isEdit, setIsEdit, isDelete, setIsDelete }) => {
  ...
  const handleDelete = (trainingId) => {
    axios
      .delete(`http://localhost:8000/trainings/${trainingId}`, {
      ...
      .then((res) => {
        ...
        setIsDelete({ ...isDelete, training: false });
    ...
    <div className="w-full p-4 pt-0">
      ...
      <div className={isOpen ? 'hidden' : 'w-full bg-white rounded-b-lg'}>
        <div className="grid grid-cols-1 justify-center items-start">
                ...
                return (
                  <Fragment key={training.id}>
                    <div className="grid grid-cols-1 lg:grid-cols-4 mt-4 pb-4 justify-center items-start px-6 border-b">
                      <div className="flex">
                        <p className="text-gray-400">{new Date(training.start_date).toLocaleDateString('id-ID', { month: 'short', year: 'numeric' })}</p>
                        <p className="text-gray-400 mx-1">
                          {''}-{''}
                        </p>
                        <p className="text-gray-400">{new Date(training.end_date).toLocaleDateString('id-ID', { month: 'short', year: 'numeric' })}</p>
                      </div>
                      <div className="col-span-1 lg:col-span-2">
                        <p className="text-lg">{training.title}</p>
                        <p className="text-gray-700">{training.organizer}</p>
                      </div>
                      <div className="order-first text-end lg:order-last space-x-2">
                        ...
                        <button
                          onClick={() => {
                            setIsEdit({ ...isEdit, training: true });
                            setCurrentId(training.id);
                          }}
                        >
                            ...
                            <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-lg transition-all sm:my-8 sm:w-full sm:max-w-lg">
                              ...
                              <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                <button
                                  ...
                                  onClick={() => {
                                    setIsDelete({ ...isDelete, training: true });
          ...
          {isAdd.training && !isEdit.training && <TrainingForm isAdd={isAdd} setIsAdd={setIsAdd} currentId={currentId} setCurrentId={setCurrentId} />}
          {isEdit.training && !isAdd.training && <TrainingForm isEdit={isEdit} setIsEdit={setIsEdit} currentId={currentId} setCurrentId={setCurrentId} />}
        </div>
      </div>
    </div>
  );
