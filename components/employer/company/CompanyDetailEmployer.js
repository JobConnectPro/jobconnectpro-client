...
const CompanyDetailEmployer = ({ res }) => {
  ...
  const handleUpdate = () => {
    router.push(`/employer/companies/edit/${company.id}`);
  };
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-md">
              <div className="mb-4">
                <input
                  ...
                  className={`shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline ${errors.logo ? 'border-red-500' : ''}`}
                />
          ...
          filteredJobs.map((job) => (
            <div key={job.id}>
              ...
              <div>
                ...
                <div>
                  <Link href={`/employer/job/${job.id}`}>Detail</Link>
                </div>
              </div>
            </div>
          ))
